...
- > **[ОСНОВНЫЕ ТЕМЫ](#основные-темы)**
    1. > **[ПЕРВИЧНАЯ НАСТРОЙКА](#первичная-настройка)**
    1. > **[ОСОБЕННОСТИ И ОСОБЫЕ ФАЙЛЫ](#особенности-и-особые-файлы)**
    1. > **[КОММИТЫ](#коммиты)**
    1. > **[ВЕТКИ](#ветки)**
    1. > **[СЛИЯНИЕ ВЕТОК](#слияние-веток)**
    1. > **[РЕСЕТЫ](#ресеты)**
    1. > **[ПРОСМОТР](#просмотр)**
    1. > **[ПЕРЕБАЗИРОВАНИЕ](#перебазирование)**
    1. > **[УДАЛЕННЫЕ РЕПОЗИТОРИИ](#удаленные-репозитории)**
...
`.gitignore` - данный файл позволяет игнорировать большинством ф-ций 
(например, `git status`, `git add .` и др.) файлы и папки, перечисленные
в нем ч/ перенос строки.
...
вместо `<filename>` указать `.`, то добавит все файлы.
...
глобальный файл .gitignore (см. "Особенности и особы файлы") для всех
проектов данного поль-ля ОС.
...
`.gitignore` - данный файл позволяет игнорировать большинством ф-ций 
(например, `git status`, `git add` и др.) файлы и папки, перечисленные
в нем ч/ перенос строки. Помещается в корень проекта. Кроме того, при
наличии одинаково названных папок или файлов в разных директориях
...
практикой будет исп-ть каждый раз перед добавлением ч-л в индекс.
________________________________________________________________________
...
удаляет его из индекса. Если вместо `<filename>` указать `.`, то добавит
все файлы.
...
загруженных в промежуточный индекс изменений. Не добавляет новые файлы.
...
приведены его исп-я в рамках темы коммитов.

`git checkout <address>` - позволяет посмотреть проект в состоянии
коммита по адресу. Но крайне важно, что в результате исп-я получается
безветочное состояние, поэтому нужно исп-ть аккуратно, иначе приведет к
созданию безымянной ветки (отделенная HEAD).
...
`git branch -f <name> <address>` - переместить ветку name, если она
существует, или насильно создать ветку по адресу. Адрес можно задать ч/
имя др. ветки.
...
в отслеживаемых индексом файлах до состояния последнего коммита.
Представляет собой исп-е упомянутого выше флага `-f` команды
`git checkout` на тек. ветке. И, кроме того, является полным аналогом
...
`git cherry-pick <address> <flag>` - скопировать изменения в коммите на
текущую ветку (скопировать коммит). Можно копировать несколько коммитов
или задать диапазон.
...
- `--squash` - берет все изменения из сливаемой ветки и применяет к
    ...
    случай отсутствия необх-ти в истории создания данной фичи.
...
2. `git checkout --ours/--theirs <file>` - позволяет решить конфликта
    путем выбора файла из принимающей или сливаемой ветки соотв.
    `--merge` - версия с конфликтом.
...
`<addr>^^` или `<addr>^2^` для 1-ого родителя родителя 1 или 2 соотв.
...
`--pretty=fuller` - выдать более полную информацию, но для пар-ра
...
`git show :/<words>` - показать самый свежий коммит со словами words в
описании.
________________________________________________________________________
...
`git log <br1>..<br2> <flag>` - вывести коммиты ветки br1 после ее
отхождения от ветки br2. Флаг `--boundary` даст вдобавок коммит
разделения.
...
- `-P` - включает перл-совместимость, что позволяет исп-ть знак ИЛИ
    внутри текста words. Например, `'say(Hi|Bye)'`.
...
`git log -L :<func_name>:<file>` - поиск коммитов с изменениями в ф-ции
func_name в файле file.
...
файле. Имеется параметр-флаг `--date`. `N,M` - строки с N до M.
...
`git rebase <addr> <br>` - переместить ветку br с ее основания на addr
(т.е. перебазировать). Стоит исп-ть с осторожностью, т.к. есть ряд
потенциальных проблем.

*Флаги*:

- `-x '...'` - запуск команды после каждого перебазированного коммита.
    Обычно исп-ся для тестов.
...
- `-i` - интерактивное перебазирование, где можно задать отдельные
    действия для каждого перебазируемого коммита, в каком порядке и т.п.
...
3. `git rebase -i --autosquash` - ребейз на себя + соединятся оригинал и
    фикс.
________________________________________________________________________
...
`git remote add <name> <addr>` - задать для проекта удаленный
репозиторий addr под именем name. Мб несколько, для основного обычно
исп-ют имя `origin`.
...
- `--mirror` - склонировать без возм-ти редактировать к-л файлы или
    ветки. Исп-ся для создания "зеркала" основного репозитория.
...
- `-f` - произвести загрузку насильно, игнорируя ошибки и
    предупреждения. Исп-ть осторожно.
- `-u` - хороший вариант для загрузки новой ветки, упрощающая гиту
    установку связей м/ локальной и удаленной версией и ветками в них.
