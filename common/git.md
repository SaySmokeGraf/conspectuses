...
`git remote add origin <addr>` - задать для проекта соответствующий
онлайн репозиторий addr.

`git push origin <branch>` - загрузить ветку branch в репозиторий.

`git clone <addr> <new_name>` - скопировать в текущую папку проект.

`git pull` - для подзагрузки новых изменений в проекте с их применением
в рабочей папке.
________________________________________________________________________
...
### GITHUB ###
<a id="anchor_github"></a>

Хотя раздел назван "GitHub", это относится и к прочим онлайн платформам
для гит-репозиториев.
...
`git clone <addr> <new_name>` - скопировать в текущую папку проект. В
конце new_name позволяет задать новое имя папке. Без него будет так же,
как в клонируемом репозитории.
________________________________________________________________________

`git fetch` - для подзагрузки новых изменений в проекте, но без их
применения на рабочей папке.

`git pull` - для подзагрузки новых изменений в проекте с их применением
в рабочей папке. По сути = `git fetch` + `git merge`.
________________________________________________________________________
...
- GIST позволяет сохранять фрагменты часто используемого кода вручную на
    GitHub.
- Fork - ответвление, создать копию проекта (мб чужого) на своей
...
1. [YT Плейлист] **Еще один курс по git** (менее подробный, но дополняет
    предыдущий):
