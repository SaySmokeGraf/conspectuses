...
позволяют ему взаимод-ть с собой, своими атрибутами и методами или др.
частью кода (будь то переменные, ф-ции или др. объекты).

*Класс* - шаблон для создания объектов.

Код в ООП должен соотв-ть основным принципам ООП, описанным в разделе
"Принципы".
________________________________________________________________________
...
- Сложность в освоении больше, чем у ФП.
- Громоздкость. Там, где в ФП хватит одной ф-ции для небольшой
    программы, в ООП нужно создать класс, объект, методы и атрибуты. Но
    для больших программ это оборачивается плюсом в виде
    структурированности.
- Низкая производ-ть. Объекты потребляют больше памяти, чем простые
    ф-ции и переменные.
...
1. *Инкапсуляция* - данные, необходимые для работы объекта, должны
    ...
    списке весов и уронов мечей за пределами экземпляров. Кроме того,
    нельзя давать поль-лю своевольно менять данные хар-ки, но смотреть в
    любой момент - нужно.
    ...
    все также относится к контролю доступа к данным, который
...
2. *Наследование* - механизм, аналогичный реальному биологическому
    ...
    После такого объявления класс EnchantedSword инициализируется теми
...
3. *Полиморфизм* - принцип взаимозаменяемости схожих классов, говорящий,
    ...
    можно просто вызвать метод normal_strike вне зав-ти от того, что
    экипировано, хотя реализации этих методов и в целом классы оружия
    разные и имеют свои особенности.
________________________________________________________________________
...
указываюся. Но кроме них есть еще несколько полезных принципов,
...
3. **YAGNI** - You Aren't Gonna Need It - "тебе это не понадобится". Не
    ...
    раньше, но сейчас не исп-ся. Писать и оставлять только то, что нужно
    сейчас. Захламленный неиспользуемыми блоками код усложняет его
    поддержку.

4. **BDUF** - Big Design Up Front - "глобальное проектирование прежде
...
6. **Бритва Оккама** - "не следует множить сущее без необходимости".
    ...
    сущности без необх-ти. Это относится как к неиспользуемым сущностям
    на будущее, так и используемым, но без необх-ти на то усложняющим
    код - "сущность ради сущности".
________________________________________________________________________
...
- Хорошая практика по OCP: создать классы "Меч" и "Топор" с принимаемой
    хар-кой "урон" (для описания разных мечей и топоров одним соотв.
    классом и по необх-ти - наследоваться от них для особых случаев и
    накладывания статусов на противника). После этого в соответствии с
    полиморфизмом ООП добавить метод или св-во в эти классы,
    возвращающее единообразно все необходимое для атаки, а из метода
    "атаковать" игрока вызывать этот метод или св-во. Тогда вне зав-ти
    от кол-ва оружий метод "атаковать" игрока не будет меняться, а
    функционал оружий будет лежать в определенном месте, упрощая
    поддержку и читаемость.
________________________________________________________________________
...
не исп-ют. Создавайте узкоспециализированные интерфейсы. Вместо одного
большого интерфейса лучше создавать несколько маленьких - каждый со
своей задачей. За счет такого подхода классы могут реализовывать только
...
- Плохая практика по ISP: создать класс Weapon, который будет содержать
    ...
    но для мечей пар-р "кол-во патронов" и метод "перезарядка" не нужны
    и будут болтаться без дела.
...
др. принципами (ООП, KISS, YAGNI и т.д.), что нарушение одного ведет к
нарушению другого или даже нескольких других. Т.о. SOLID частично
дополняет, частично вбирает в себя и другие принципы и идет с ними
неразрывно.

С другой стороны, в некоторых местах он может конфликтовать с др.
...
нет. Но тем не менее, в Pyhton иногда имеет место быть.
...
Проще говоря, позволяет вместо мн-ва классов с различными комбинациями
св-в позволяет создать класс объекта и ряд классов св-в, а затем
комбинировать их посредством атрибутов класса объекта.
...
который дает возможность последовательно обходить элементы составных
объектов, не раскрывая их внутреннего представления.

<u>Используется</u>:

- Когда у вас есть сложная структура данных, и вы хотите скрыть от
    клиента детали ее реализации (из-за сложности или вопросов
    ...
    доступ к коллекции, но и защищает ее данные от неосторожных или
...
- Когда вам нужно иметь несколько вариантов обхода одной и той же
    ...
    могут иметь довольно объемный код. Этот код будет захламлять все
...
- Когда вы не можете повторно исп-ть класс, поскольку он зависит от уймы
    ...
    связи с другими компонентами, а все их общение происходит косвенно,
    ч/ объект-посредник.
...
- Когда вам приходится создавать мн-во подклассов компонентов, чтобы
    ...
    создать подкласс посредника и поменять в нем связи м/ компонентами.
...
который создает механизм подписки, позволяющий одним объектам следить и
реагировать на события, происходящие в других объектах.

<u>Используется</u>:

- Когда после изменения состояния одного объекта требуется что-то
    сделать в других, но вы не знаете наперед, какие именно объекты
...
- Когда одни объекты должны наблюдать за другими, но только в
    определенных случаях. Издатели ведут динамические списки. Все
...
Извне создается впечатление, что изменился класс объекта. Основан на
концепции машины состояний, или конечных автоматов.

<u>Используется</u>:

- Когда у вас есть объект, поведение которого кардинально меняется в
    зав-ти от внутреннего состояния, причем типов состояний много, и их
    код часто меняется. Паттерн предлагает выделить в собственные классы
    все поля и методы, связанные с определенными состояниями.
...
- Клиент должен знать, в чем состоит разница между стратегиями, чтобы
    выбрать подходящую.
________________________________________________________________________
...
- Когда подклассы должны расширять базовый алгоритм, не меняя его
    ...
    определенные шаги алгоритма через наследование, не меняя при этом
...
- Когда у вас есть несколько классов, делающих одно и то же с
    ...
    суперкласс и оформить в нем главный алгоритм в виде шагов.
