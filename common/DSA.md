...
- > **[КРАТКАЯ ТЕОРИЯ](#краткая-теория)**
    1. > **[ОБЩЕАЛГОРИТМИЧЕСКОЕ](#общеалгоритмическое)**
    1. > **[МАТВЫКЛАДКИ](#матвыкладки)**
        - > **[Формулы](#формулы)**
        - > **[Методы](#методы)**
    1. > **[СЛОЖНОСТЬ](#сложность)**
    1. > **[АЛГОРИТМИЧЕСКИЕ И МАТЕМАТИЧЕСКИЕ ФИЧИ](#алгоритмические-и-математические-фичи)**
- > **[PYTHON](#python)**
- > **[АЛГОРИТМЫ](#алгоритмы)**
    ...
    1. > **[СОРТИРОВКА](#сортировка)**
        ...
        - > **[Устойчивая сортировка подсчетом](#устойчивая-сортировка-подсчетом)**
        ...
        - > **[Прочие алгоритмы сортировки](#прочие-алгоритмы-сортировки)**
    1. > **[ПРЕФИКСНЫЕ СТРУКТУРЫ](#префиксные-структуры)**
        - > **[Префиксный список (сумма)](#префиксный-список-сумма)**
        - > **[Разреженная таблица](#разреженная-таблица)**
        - > **[Дерево отрезков](#дерево-отрезков)**
        - > **[Дерево Фенвика](#дерево-фенвика)**
    1. > **[ДВА УКАЗАТЕЛЯ](#два-указателя)**
    1. > **[СКОЛЬЗЯЩЕЕ ОКНО](#скользящее-окно)**
    1. > **[РАЗДЕЛЯЙ И ВЛАСТВУЙ](#разделяй-и-властвуй)**
    1. > **[ПОИСК ДЕЛЕНИЕМ ОТРЕЗКА](#поиск-делением-отрезка)**
        - > **[Бинарный поиск](#бинарный-поиск)**
        - > **[Тернарный поиск](#тернарный-поиск)**
        - > **[Поиск методом золотого сечения](#поиск-методом-золотого-сечения)**
        - > **[Более экзотические методы](#более-экзотические-методы)**
    1. > **[СОРТИРОВКА СОБЫТИЙ](#сортировка-событий)**
    1. > **[ДИНАМИЧЕСКОЕ ПРОГРАММИРОВАНИЕ](#динамическое-программирование)**
        - > **[С одним параметром](#с-одним-параметром)**
        - > **[С двумя параметрами](#с-двумя-параметрами)**
    1. > **[ЖАДНЫЙ АЛГОРИТМ](#жадный-алгоритм)**
        - > **[Задача о рюкзаке](#задача-о-рюкзаке)**
    1. > **[БИТОВЫЕ ОПЕРАЦИИ](#битовые-операции)**
    1. > **[ХЕШИ ДЛЯ СТРОК](#хеши-для-строк)**
        - > **[Сравнение полиномов](#сравнение-полиномов)**
        - > **[Сравнение строк и подстрок](#сравнение-строк-и-подстрок)**
        - > **[Задачи на хеши](#задачи-на-хеши)**
    1. > **[ПЕРЕБОР](#перебор)**
        - > **[Поиск с возвратом (backtracking)](#поиск-с-возвратом-backtracking)**
        - > **[Методы оптимизации перебора](#методы-оптимизации-перебора)**
    1. > **[СПЕЦИАЛЬНЫЕ АЛГОРИТМЫ](#специальные-алгоритмы)**
        - > **[Алгоритм Хаффмана](#алгоритм-хаффмана)**
        - > **[Исправляющие коды Хэмминга](#исправляющие-коды-хэмминга)**
        - > **[Алгоритм Лемпела-Зива-Велча](#алгоритм-лемпела-зива-велча)**
        - > **[Преобразование Барроуза-Уиллера](#преобразование-барроуза-уиллера)**
        - > **[Алгоритм А\*](#алгоритм-а)**
        - > **[Метод Монте-Карло](#метод-монте-карло)**
- > **[СТРУКТУРЫ ДАННЫХ](#структуры-данных)**
    1. > **[МНОЖЕСТВО (ХЕШ-ТАБЛИЦА)](#множество-хеш-таблица)**
        - > **[Хеш-таблицы в Python](#хеш-таблицы-в-python)**
    1. > **[СВЯЗНЫЕ СПИСКИ](#связные-списки)**
        - > **[Особые использования списков](#особые-использования-списков)**
    1. > **[СТЕК](#стек)**
    1. > **[ОЧЕРЕДЬ](#очередь)**
    1. > **[ДЕК](#дек)**
    1. > **[КУЧА](#куча)**
    1. > **[БИНАРНОЕ ДЕРЕВО (ПОИСКА)](#бинарное-дерево-поиска)**
        - > **[Предварительная информация](#предварительная-информация)**
        - > **[Описание бин. дерева](#описание-бин-дерева)**
        - > **[Основные операции](#основные-операции)**
        - > **[Балансировка](#балансировка)**
        - > **[Дополнительно](#дополнительно)**
    1. > **[ДРУГИЕ ДЕРЕВЬЯ](#другие-деревья)**
    1. > **[ГРАФ](#граф)**
        - > **[Описание графа](#описание-графа)**
        - > **[Обход в глубину](#обход-в-глубину)**
        - > **[Обход в ширину](#обход-в-ширину)**
        - > **[Агоритм Дейкстры](#агоритм-дейкстры)**
        - > **[Граф состояний](#граф-состояний)**
    1. > **[ПЕРСИСТЕНТНЫЕ СТРУКТУРЫ](#персистентные-структуры)**
...
выполнения операций для решения задач определенного типа. Обладает след.
...
(например, см. "Динамическое программирование"), а другие ч/ нее
...
2 \* N \* logN при N >= 2\*\*500, что нереалистично большое число. Т.о.,
порой важно учитывать константы и прочее, а не только сложность. Но в
условиях решения задач по алгоритмам такая ситуация крайне редка.
...
Итого, на самом деле у алгоритмов есть множество других различных
критериев оценивания, например:
...
Полезные ф-ции для алгоритмов и для понимания дальнейших выкладок:

- `set(<smth>)` - сделать мн-во уникальных эл-тов из др. итерируемого
    объекта smth. Можно подробнее изучить мн-ва в Python при желании.
- `'<delim>'.join(<list>)` - собрать строку из эл-тов списка list с
    разделителем delim.
...
(Для второй вариации задачи проверки на равенство -1 не нужно, либо, для
сведения к первой вариации, можно перебирать с конца в начало).
...
- Любой алгоритм сортировки можно написать так, что он будет
    неустойчивым.
...
Алгоритмическая сложность у этой задачи O(N\*\*2), но она так высока
...
Идея *сортировки слиянием* (merge sort):
...
процесс в сумме будет занимать K \* O(N), или O(N \* K), где K - кол-во
запросов.
...
Во-вторых, о чем важно помнить во всех языках - это о различии размеров
...
- O(N): Посчитать префиксные суммы. Одинаковые префиксные суммы - сумма
    ...
    можно использовать словарь для подсчета каждой префиксной суммы, что
    в данном решении префиксная структура и есть.
...
расчетов. Для большего понимания: см. задачи в "Префиксных списках".
________________________________________________________________________
...
Хранение - по аналогии с кучей (см. "Структуры данных" -> "Куча"), т.е.
в виде одномерного списка размером 2N - 1, где N - размер дополненного
до степени двойки оригинального списка. Тут для i-ого эл-та родитель
...
- Пробегаем рекурсивно по аналогии с поиском и отвечаем на вопросы:
    - [L, R] покрывает зону ответственности узла полностью? Если да, то
        ...
        выполнят эту операцию на себе и своих детях, и сразу возвращаем
...
Таким образом можно прибавлять, присваивать, умножать и т.д.
________________________________________________________________________
...
ссылкам в "Источниках и доп. материалах". Кроме того, там можно почитать
подробнее в целом. Но в общем стоит подметить и из этих материалов, и из
Leetcode, что скользящее окно - довольно важный алгоритм, особенно для
собеседований.
________________________________________________________________________
...
*Бинарный (двоичный) поиск* (binary search) - поиск посредством
...
```python
# check здесь - некоторая ф-ция проверки, передаваемая в ф-ции поисков,
# а checkparams - пар-ры для ф-ции проверки, если необходимы.
...
читабельность. Опять же, тезис о разумности оптимизации - см.
"Сложность".
________________________________________________________________________
...
т.к. скорость первого может только увеличиваться по ходу обгонов и
изменения велосипедистов на первом месте, а последнего по аналогии -
только уменьшаться. Получается кусочно-линейная ф-ция. Если
`dist(t + eps) > dist(t)`, то ф-ция растет и надо сдвинуть правую
границу поиска, иначе - левую.

> **ЗАМЕТКА!** Хотя в решении используем бинпоиск по производной, здесь
может исп-ся (и, кроме того, является более специализированным под такие
случаи) тернарный поиск.
...
*Тернарный (троичный) поиск* (ternary search) - поиск, применяемый для
...
логарифма можно утверждать, что существует константа C, такая что
log3/2 N = C \* logN, что ведет к сложности O(logN). Тем не менее, более
конкретная сложность понадобится при сравнении разных методов на основе
деления отрезка.

Рассмотрим на примере поиска максимума, минимум симметричен. Алгоритм
троичного поиска максимума пос-ти:

1. Разбить диапазон поиска на 3 части, сравнить значения на границах
    этих частей.
2. Если значения возрастают, изменить левую границу диапазона на меньшую
    границу разбиения. Если значения убывают, изменить правую границу
    диапазона на большую границу разбиения.
3. Продолжать, пока не останется 3 элемента. Затем возвращаем максимум
    из них.
...
Алгоритм троичного поиска точки максимума ф-ции:
...
2. Если значения возрастают, изменить левую границу диапазона на меньшую
    границу разбиения. Если значения убывают, изменить правую границу
    диапазона на большую границу разбиения.
3. Продолжать, пока не достигнем желаемой точности. Максимум функции
...
т.к. его сложность меньше, рассматривая конкретику (см. "Источники и
доп. материалы").

Но в случае поиска экстремума унимодальной ф-ции все меняется, т.к. для
бинарного поиска требуется доп. действие на каждом шаге - подсчет
производной (напомним, что бинпоиск рассчитан на монотонную ф-цию или
пос-ть, а не унимодальную). Таким образом получается, что тернарный
поиск затратит меньше времени, т.к. его сложность меньше.
________________________________________________________________________

### <u> Поиск методом золотого сечения </u> ###

*Метод золотого сечения*, или "золотой" поиск (golden search) - поиск,
применяемый для нахождения максимума или минимума унимодальной ф-ции или
пос-ти. Представляет собой модификацию идеи тернарного поиска, и
относительно него позволяет уменьшить число операций за итерацию с 2 до
1.
...
> **ЗАМЕТКА!** В более стром смысле сложность не logN, а log_phi N. А
еще более конкретно - с константами. Тем не менее, как и с обычным
тернарным поиском, сводится к O(logN). Но стоит помнить при сравнении
разных методов поиска.
...
(подробнее - см. "Источники и доп. материалы") подходящие соотношения,
получим, что такое разбиение возможно при расстояниях по золотому
сечению.
...
Несмотря на результирующую сложность, в реальности на смену доп.
подсчетам пришло большее число записей в переменные, что вызывает
спорную ситуацию о том, что лучше: тернарный или "золотой" поиск. Но
если это опустить, то "золотой" поиск лучше по сложности подсчетов
примерно в 2.37 раз (см. "Источники и доп. материалы").
________________________________________________________________________

### <u> Более экзотические методы </u> ###

Здесь представлены довольно редко встречающиеся методы поиска,
основанные на делении отрезка. Они могут быть в некоторых случаях не
хуже, а иногда даже быстрее своих популярных собратьев выше, но при этом
по тем или иным причинам используются реже. Для собеседования будет
плюсом просто знать их, но задач на них не будет. В жизни тоже
маловероятно пригодятся, но тем не менее, для общего ознакомления все же
...
1. Как и в тернарном поиске, рекурсивно вычисляем минимум функции на
    ...
    B = (L + R) / 2 и C = (L + 3R) / 4.
2. Посмотрим, какое из значений в этих трёх точках самое маленькое. При
    этом, надо заметить, что если f(A) <= f(B), то это уже означает,
    что левое значение - самое маленькое, если f(С) <= f(B) - то правое;
    в остальных случаях - среднее.
3. Если самое маленькое - левое, то запускаемся рекурсивно на отрезке
...
вычислять: оно может и не пригодиться.
...
Таким образом, количество требуемых итераций в 1.71 раз меньше, а кол-во
вычисляемых значений за итерацию в среднем меньше, чем в тернарном
поиске.
...
дополнительно дважды получить эл-т пос-ти.
________________________________________________________________________

## СОРТИРОВКА СОБЫТИЙ ##

*Сортировка событий* (event sort) - это не столько алгоритм, сколько
подход к решению задач. Суть заключается в отношении к полученным данным
как к событиям, где это уместно, и их последующая обработка.
...
событие "вход" должно происходить раньше события "выход". Для этого тип
события, который будем выражать числом, для "входа" должен быть меньше.

Сортируем события, проходим по ним и увеличиваем или уменьшаем счетчик,
исходя из типа события. Записываем в максимум, если оно больше текущего
максимума.

> **ЗАМЕТКА!** Коды событий в программе ниже заданы хардово (-1 и 1).
Для расширяемости и читаемости лучше задавать их в отдельных константах.
...
обработаны, а события выхода - еще нет. При наступлении события "вход
...
Решить двумя проходами: сначала определить минимальное число авто, а
затем выявить номера машин в таком случае. Тогда мн-ва не копируются и
решение не заваливается, как предыдущее.
...
*Динамическое программирование* (dynamic programming) — это способ
решения сложных задач путём разбиения их на более простые подзадачи.
Метод сводит к минимуму лишние вычисления и оптимизирует
производительность, потому что разбивает большую задачу на более мелкие
подзадачи, решает каждую из них и сохраняет результаты.
...
Рекурсивное с учетом базы и перехода.

**Решение 2**:

Решение 1 может быть оптимизировано, т.к. часто приходится пересчитывать
одни и те же конструкции. Например, F(10) = F(9) + F(8), а
F(9) = F(8) + F(7), и т.к. рекурсивно все вызывается, то расчет F(8)
будет произведен дважды, а это целое дерево вызовов.

Решение: подтянуть кэширование в рекурсию - *рекурсия с мемоизацией*. На
жаргоне называется *ленивое динамическое программирование*.
...
затратная операция. Без рекурсии решается:

- Циклом с заполнением массива длиной n + 1 и заданными 0-ым и 1-ым
    значением от 2-ого эл-та и до n-ого, если нужно запомнить предыдущие
    значения.
- Перекладыванием значений в пару переменных, если запомнинать не нужно.
________________________________________________________________________
...
*База*:
- На 0-ую ступеньку мы можем попасть 1 способом.
- На 1-ую ступеньку мы можем попасть 1 способом.
- На 2-ую ступеньку мы можем попасть 2 способами.

*Переход*:
- На n-ую ступеньку мы можем попасть с n - 1, n - 2, n - 3 ступенек, на
    которые можем попасть A, B, C способами. Тогда на n-ую ступеньку мы
    можем попасть A + B + C способами.

Таким образом, получаем мат. индукцию, по которой начиная с 3-ей
ступеньки считаем до необходимой n-ой.
________________________________________________________________________
...
учетом ее самой. Пусть `cost` - список стоимостей ступенек, тогда
`cost[0] = 0`.
...
Для каждой позиции запоминаем длину самой большой подпос-ти, которая
...
Сложность: O(N\*\*2). И да, это нормально. Задачу можно решить за
...
Таким образом, лучше всего будет вести 2 аналога dp - S[i] и M[i] -
оптимальное представление числа i в виде суммы и произведения
соответственно.

Кроме того, для получения результата оптимальнее таскать с собой список
строк ans[i] с выражением в ней для i-ого.
...
- С ans больше сложностей, т.к. конкатенация, перезапись строки и проч.
    ...
    сложность алгоритма в O(N\*\*3). Тут можно либо таскать списки
    символов вместо строк, либо, что значительно быстрее, запоминать, из
    чего мы получаем самый оптимальный вариант, а затем восстанавливать
    ответ.
________________________________________________________________________
...
так и далекими друг от друга или особыми случаями - параметры разной
природы, динамика по подстрокам.

Вычисляется в двумерном массиве, в отличие от одномерного случая.
...
верхнего угла в правый нижний и собирает монетки. Она может двигаться
...
Поиск через хранение 2-х пар-ров - максимума денег и минимума долгов на
пути - не выйдет, т.к. выравнивая динамику по максимуму денег мы можем
где-то посреди процесса прийти в ситуацию, что денег-то много
получилось, но и был момент, когда долг был не минимальный. И наоборот,
выравниваясь по минимуму долга, мы можем прийти в момент, что не запасли
достаточно, хотя могли, пусть и с чуть большим долгом, и в итоге
следующим ходом влезли в еще большие долги, чем могли бы.
...
> **Задача 2**. Найти наибольшую общую подпос-ть (НОП) двух пос-тей.

> **ПРИМЕР:** НОП(АБВАБ, ГВАВВБ) = АВБ.

**Решение**:

Как бы собираем таблицу с символами одной пос-ти (назовем ее `pcol`) в
тайтлах столбцов, а другой пос-ти (`prow`) - строк.

dp[i][j] хранит наибольшую общую подпос-ть pcol[0:i + 1] и
prow[0:j + 1].
...
> **ПРИМЕР:** ВОЛЫНКА и ВАЛЕНОК требуют 4 операции: символ 1 в
результате (в нуль-нумерации) - замена О на А; символ 3 - замена Ы на Е;
символ 5 - вставка О; символ 8 - удаление А.
...
2. Пробежки по делителям числа R - L + 1 и сравнение разбиения подстроки
    на полученное число подстрок с целью выделения таких случаев, как в
    примере YESYESYES -> 3(YES).
________________________________________________________________________

## ЖАДНЫЙ АЛГОРИТМ ##

*Жадный алгоритм* (greedy, "жадник", "жадина") - действуем наилучшим
образом на данный момент, не задумываясь о будущем.
...
> **Задача 1**. Известна стоимость проезда одного км для N такси, а
также сколько км ехать N людям от текущей точки до своих домов.
Рассадить людей по такси так, чтобы общая сумма была наименьшей.
...
"Тренировках 7.0" приводит док-во, приведу его в качестве примера образа
мышления при док-ве, в дальнейших задачах док-ва приводить не буду.
...
`X * Q + Y * P`. При этом `P > Q`, `Y > X`. Тогда эта сумма не
минимальна, т.к. `X * P + Y * Q` меньше. Тогда новое решение не
оптимально, т.е. неправильное.
________________________________________________________________________
...
Песок делимый, поэтому можно брать не весь песок одной ценности, в
отличие от задачи о рюкзаке.
________________________________________________________________________

### <u> Задача о рюкзаке </u> ###

*Задача о рюкзаке* (knapsack problem, "рюкзак") стоит несколько
особняком от "жадины", т.к. иной подход, образ мышления и области
применения ("рюкзак" работает только для дискретных значений), но, тем
не менее, есть общие черты. Подход, используемый в "рюкзаке", проще
всего описать на классической задаче о рюкзаке, представленной далее.
...
> **ЗАМЕТКА!** Пробегаем именно справа налево, чтобы не научиться
собирать что-то несколькими экземплярами одного предмета. Если так можно
и предметов каждого веса безграничное кол-во, то пробегаем слева
направо.
...
*Битовые операции* (bit operations) как алгоритмическая тема позволяет
ускорить работу кода или привести к меньшей пространственной сложности
там, где это необходимо. Используется в ряде специальных алгоритмов (см.
"Специальные алгоритмы" -> "Исправляющие коды Хэмминга", "Алгоритм
Лэмпела-Зива-Велча", а также "Префиксные структуры" -> "Дерево Фенвика"
как тренировку исп-я битовых операций в алгоритмической стезе).
________________________________________________________________________
...
- Определение i и k - номера эл-та в массиве и номер бита в
    нуль-нумерации соответственно.
...
*Сравнение полиномов* - сравнение всех соответствующих коэффициентов
двух полиномов, и они считаются равными, если `a[i] == b[i]` для всех i
от 0 до N - 1, где a, b - наборы коэффициентов первого, второго полинома
...
самом деле незначительно быстрее, а в большинстве случаев - даже
медленнее.
...
1. Во-первых, возведение x в степень, особенно в большую - дело крайне
    ...
    Т.е. можно посчитать сначала `res = a * x + b`, потом
    `res = res * x + c`, а затем `res = res * x + d`. Решение за O(N).
...
2. Во-вторых, может быть переполнение в одних языках и длинная
    ...
    своеобразное хеширование поиском остатка от деления (в случае
    возникновения трудностей с пониманием данного аспекта - см.
    "Структуры данных" -> "Множества"), или из математики - *кольцо*
    *вычетов по модулю*.
...
3. В-третьих, используя п.2, понимаем, что вер-ть коллизии становится
    ...
    - x и p следует делать взаимно простыми.
    ...
    - x важно брать не слишком большим для ускорения расчетов, но не
        ...
        ориентиром будет ближайшее простое число, большее длины
        алфавита, или и вовсе длина алфавита + 1. Например, для ASCII
        это 257.

    - p стоит брать достаточно большим, чтобы минимизировать шанс
        коллизии, но так, чтобы это все еще не вызвало проблем с
        переполнением или длинной арифметикой (не более, чем
        максимальное число в данном типе данных // 2). Хороший пример
        для int64 (и для Python до наступления длинной арифметики):
        p = 10\*\*9 + 7.
...
Кроме того, применяем вычет по модулю. Получаем сложность O(N \* K). Но
помним про коллизии.
________________________________________________________________________
...
1. Это действие надо считать по модулю p, не забывая про переполнение.
    Т.о. правильная формула: `H = (h[i] - h[i - L] * x**L) % p`.
2. Тут может возникнуть проблема с отрицательной разницей, но модуль p
    самостоятельно решает проблему в таких ситуациях. Тем не менее, это
    не всегда есть хорошо. Исправление такой ситуации возможно при
    сравнении двух хешей, см. далее.
...
тогда лучшим решением (за исключением частных случаев, см. "Задачу о
рюкзаке") будет решить задачи с помощью полного перебора, пускай это и
выглядит неэффективно по времени. Кроме того, перебор используется для
тестирования при проверке верности алгоритма на разных наборах данных -
...
Одними из наиболее известных применений алгоритма backtracking являются
задача о расстановке ферзей на шахматной доске так, чтобы они не били
друг друга, а также задача коммивояжера (см. следующий подраздел).
Другие примеры включают задачи о путешествии волнового алгоритма, о
поиске пути в графе и о комбинаторной оптимизации.
...
2. Проверка, где текущий выбранный элемент или состояние проверяется на
    ...
    - Если нет, то возврат False и возвращение к выбору эл-та на
...
время кол-во эл-тов N будет порядка десятков, и в таком ключе уже
становится важна константа, т.к. O(1) может сравниться и даже стать хуже
O(N). Поэтому каждая оптимизация должна быть проверена более
внимательно, действительно ли оптимизация оптимизирует. Также это ведет
к тому, что подобные задачи для больших N неразрешимы в адекватные
временные рамки на данный момент, и поэтому для больших N приходится
придумывать полиномиальные или еще более быстрые решения, но допускающие
ошибку. Например, см. "Хеши для строк".
________________________________________________________________________
...
вызывается эта же ф-ция для следующей ячейки в состоянии, когда в
текущую был поставлен ферзь, и для состояния, когда в текущую ферзь
поставлен не был. Не углубляться в рекурсию далее, если ячейка
последняя. Если углубления не происходит, проверяем случай на то, бьют
ли попарно ферзи друг друга или нет. Возвращает 1 или 0, а выше по
рекурсии суммируется.

Сложность: O(N\*\*2 \* 2\*\*(N\*\*2)).
...
100%, но для общего интереса, общего пула задач и их возможных решений
можно рассмотреть.
________________________________________________________________________

### <u> Алгоритм Хаффмана </u> ###

*Алгоритм Хаффмана* (Haffman algorithm) - жадный алгоритм оптимального
префиксного кодирования алфавита с минимальной избыточностью. Проще
говоря, позволяет сопоставить более часто встречающимся символам более
короткий код, причем вся кодировка букв должна быть такова, что по
подряд записанным кодам букв без разделителей можно было точно
восстановить изначальное сообщение.
...
Например, пускай ключом будет "аааабббвг". Тогда дерево:
...
*Алгоритм Лемпела-Зива-Велча* предназначен для сжатия данных. Например,
zip-файлы сжимаются алгоритмами из этого семейства (LZ).
...
- Хранить номер предыдущего слова минимальным кол-вом бит. Например, для
...
- Коды символов можно сжать с помощью кодирования Хаффмана (сериализация
    дерева Хаффмана - "Тренировки по алгоритмам 1.0" от Яндекса, Лекция
    8). В целом же - см. выше коды Хаффмана.
________________________________________________________________________

### <u> Преобразование Барроуза-Уиллера </u> ###

Данная тема относится к хешированию строк и архивации. *Преобразование*
*Барроуза-Уиллера* (BWT) само по себе - преобразование строки особым
...
*Алгоритм А\** ("А-звезда", "A-star") используется для поиска
кратчайшего пути от одной начальной точки до одной конечной.
Представляет собой модификацию алгоритма Дейкстры, основанную на
использовании идеи жадного алгоритма, но в отличие от Дейкстры алгоритм
А\* не ищет кратчайшие пути до всех точек, чем и ускоряет свою работу
относительно прародителя. Широко исп-ся в навигационных системах,
робототехнике и разработке игр.
...
*Метод Монте-Карло* (Monte Carlo method) - численный метод решения
математических задач при помощи моделирования случайных величин. В ряде
специальностей является незаменимым.
...
детектором, можно посчитать, сколько частиц попало в детектор, откуда
выяснить геометрический фактор. Метод, из очевидного, не абсолютно
точный, но позволяет получать оценки величин или модельно-эмпирические
значения, где это уместно.
...
либо крайне сложным расчетам, либо к упрощению расчетов по формулам и
потере точности. В то же время с помощью метода Монте-Карло это
становится лишь вопросом времени на моделирование.
...
**Метод Монте-Карло**:
[ссылка](https://thecode.media/monte-carlo/).
...
*Множество* (set, hash set, хеш-таблица, hash table) - математическая
структура, содержащая в себе некоторое число (\>=0) элементов.
*Мультимножество* - мн-во, куда один эл-т может входить несколько раз.
...
- Добавление в хеш-таблицу: описано выше.
- Проверка наличия элемента в хеш-таблице: вычислить хеш-функцию и
    поискать в списке, лежащему в индексе по значению хеш-ф-ции.
...
- Удаление элемента из хеш-таблицы:
    - Как из обычного списка, если порядок важен, и тогда сложность есть
        ...
        поочередного копирования элементов из i + 1 в i по всем i с
        последнего до следующего за удаляемым, а затем удаления
        последнего элемента.
...
коэффициент заполнения был не больше единицы (K / N \<= 1, или K \<= N).
...
способами за O(N \* K\*\*2) и положим получившиеся слова в множество.
Для каждого слова из текста проверим, есть ли оно в множестве, за O(1).

Итого решается за O(N \* K\*\*2 + M).
...
3. Для сохранения памяти используют дополнительную фичу. Рассмотрим на
    ...
    причинам, указанным в п.1. По сути сама хеш-таблица олицетворяется
...
*Односвязный список* (linked list) - список эл-тов, состоящих из 2
полей: ключ и ссылка на следующий (или предыдущий) элемент. Содержит
также указатели на head и tail, и опционально может содержать указатель
на текущий эл-т (либо для последующего удаления/вставки не в конец или
начало, либо для других конкретных точечных целей).

*Двусвязный список* (double linked list) - список элементов, состоящих
из 3 полей: ключ, ссылка на следующий элемент, ссылка на предыдущий
эл-т. Содержит также указатели на head и tail, и опционально может
содержать указатель на текущий эл-т (либо для последующего
удаления/вставки не в конец или начало, либо для других конкретных
точечных целей).

Преимущества: быстрая вставка и удаление эл-тов в конце и (или) начале
за O(1); динамическая расширяемость в пределах структуры данных, на
которой реализуется.
...
двусвязном списке. Но нужно быть осторожными, т.к. если связные списки
реализуются на стандартном массиве/списке, то лучше не создавать лишнюю
прослойку и написать стек, очередь и дек на стандартном массиве/списке
напрямую.

Связные списки, кроме того, бывают полезны в ряде задач и сами по себе.
В частности, в улучшении константы или функционала некоторых структур
данных или в идее персистентных структур данных.
________________________________________________________________________
...
Пара интересных моментов с задачами на связные списки:

- Один из способов решения задач при проблеме с нумерацией или
    родственными связями между узлами: создать узел dummy, дочерним
    эл-том которого будет заголовок заданного связного списка.

- Хороший метод при решении некоторых задач: создать 2 указателя,
    которые движутся с разной скоростью по связному списку.
...
### <u> Особые использования списков </u> ###

Данная подтема посвящена использованию ссылочных типов данных как идеи
для улучшения функционала или асимтотики использования ряда структур
данных, а также особых структур данных.
...
logN эл-тов, по 1 на каждый слой. Если же меняется вся левая половина
...
слова нет, если нет метки конца слова).
...
В боре есть слова (слева направо): `AB, ABD, AAA, AAG, B, C`. Других
слов нет.
...
Тогда, например, используя 3-ий вариант на массиве массивов, можно
...
эл-та X и найдя корень, перекидывать указатель эл-та X с его родителя на
корень. Тогда при следующем обращении к тому же эл-ту будет сделано не
logN действий по проходке, а всего 1.

Сложность операции объединения всех эл-тов в 1 мн-во тоже равно
O(N logN), но в остальном оно быстрее. Достигается в сумме (со всеми
описанными выше улучшениями) сложность O(log\*N).

log\*N - это с одной стороны логарифмическая сложность, но по какому-то
принципу (в данном случае с помощью улучшения константы + замены со
временем сложности O(logN) на O(1) при проверке принадлежности) данная
сложность становится меньше O(logN) и стремится к O(1). Т.о. сложность
O(log\*N) лучше O(logN), но хуже O(1).
________________________________________________________________________

## СТЕК ##

*Стек* ("стопка", stack) - структура данных на односвязном списке. В
простейшем виде должна поддерживать 2 операции - положить в стек
(`push(smth)`) и забрать (`pop()`). При складывании в стек эл-т
добавляется в конец (как бы наверх стопки), а при изъятии - достается из
конца (как бы забирается сверху стопки). Пример - детская пирамидка из
колечек на стержне. Т.е. изымается всегда то, что было сложено в стек
последним.

Реализует принцип "первый пришел - последний вышел" ("First In - Last
Out (FILO)", "Last in - First Out (LIFO)").

Стек используется много где, и однозначно все его возможные области
применения не опишешь никак, кроме как определением стека. Самые
наглядные и базовые примеры использования представлены в задачах далее.

Самая лучшая реализация - на динамически расширяемом массиве/списке, где
за O(1) будет происходить и удаление, и добавление. При этом не
потребуется дополнительная память на ссылочную структуру данных.
________________________________________________________________________
...
Рассмотрим, что есть правильная скобочная пос-ть. Для правильной пос-ти
каждая открытая скобка должна когда-либо после "открытия" закрываться, а
закрытая - когда-либо до "закрытия" открываться. Т.е. у каждой скобки
должна быть пара в виде соответствующей скобки противоположной роли.

Тогда решением будет считать "баланс": октрытая скобка - +1 к балансу,
закрытая - -1 к балансу. Баланс всегда должен быть >= 0, а в конце == 0.
Решение таким образом без стека.
________________________________________________________________________
...
Решение завести несколько "балансов" - по одному на вид скобки - здесь
уже не работает, т.к. есть случаи, когда все работает в >= 0 и
оканчивается в 0 для каждого из балансов, но в общем и целом для пос-ти
это неправильно. Например: `([)]`. Тут и понадобится решение через стек.
...
большего справа/слева - аналогично меньшему, но с другим знаком
сравнения.

> **ЗАМЕТКА!** Такой монотонно возрастающий (убывающий, невозрастающий,
неубывающий) стек называется *monotonic stack* (монотонный стек).
________________________________________________________________________

У стека также есть несколько полезных и интересных использований.

Например, с помощью стека можно заменить рекурсию. Это может
понадобиться при ограничениях на глубину рекурсии, а также из-за того,
что запуск функции есть ресурсоемкая операция. Последнее возможно через,
по сути, симуляцию рекурсии и сохраняемого ей стека данных. Подробнее
можно посмотреть на примере кода далее или в "Тренировках".
...
Идея тут в ручном выделении локальной памяти в стеке с запоминанием, до
или после рекурсивного вызова мы находимся.
...
Интересный, но крайне специфичный способ использования - расчет
арифметических выражений. Для этого представим выражение в постфиксной
записи.

Есть *префиксная*, *инфиксная* и *постфиксная* записи. Инфиксная -
привычная нам из математики, где операнды записаны по две стороны от
операции. В постфиксной записи выражения сначала записываются операнды,
а затем - операции. В префиксной же наоборот, и именно в такой записи
реализован привычный синтаксис функций в программировании.

> **ПРИМЕР:** Инфиксная: `6 + 3 * (1 + 4 * 5) * 2`. Постфиксная:
`6 3 1 4 5 * + 2 * * +`.

> **ПРИМЕР:** Инфиксная: `1 + 2 * 3`. Префиксная: `+ 1 * 2 3`. Последнее
можно прочитать как `sum(1, mul(2, 3))`, что заметно более знакомо и
понятно.

Постфиксная запись становится понятнее, если читать задом наперед и
относиться к операциям как к функциям с 2 параметрами, где один из них -
число, а оставшийся - выражение. Тогда по строке с выражением можно
пробежаться и заполнять стек, если встречаем число, и выполнять операцию
с вторым и первым числом в стеке (порядок важен для разницы и деления),
если встречаем операцию.

Для преобразования инфиксной записи выражений в постфиксную также
используется стек (промежуточный для операций) и некоторая строка или
список как ответ. По следующему принципу:
...
*Очередь* (queue) - по аналогии со стеком, эл-т складывается в конец, но
достается же из начала, т.е. достается первый добавленный эл-т из
имеющихся. В этом вся разница со стеком. В простейшем виде должна
поддерживать 2 операции - положить в конец очереди (`push(smth)`) и
забрать из начала очереди (`pop`).

Принцип: "первый пришел - первый вышел", "First In - First Out (FIFO)".

Самая лучшая реализация - на односвязном списке с указателями на начало
и конец очереди, сделанном через стандартные методы (в Python через дек
\- `collections.deque`) или через ссылочную структуру данных (например,
в Python - см. "Связные списки", реализацию с Leetcode односвязного
списка, сделать по аналогии).
________________________________________________________________________

Строение - односвязный список + указатель на начало очереди + указатель
на конец очереди. Реализация с ограничением длины списка, чтобы при
большом кол-ве операций список не разрастался и не занимал слишком много
памяти. Получается *кольцевой буфер*, где при выходе за последний эл-т
списка указатель перескакивает на начало.
...
удаления, т.к. удаление производится из начала и в классическом виде на
стандартном массиве/списке было бы нужно совершить O(N) перезаписей, что
долго. В случае кольца (или ссылочной структуры) же получаеся O(1).
...
Можно на односвязном списке.
________________________________________________________________________

## ДЕК ##

*Дек* (deque, очередь с двумя концами, **d**ouble **e**nded **que**ue) -
...
Самая лучшая реализация - через стандартные методы (в Python -
`collections.deque`) или ссылочные типы данных (например, в Python - см.
"Связные списки", реализацию с Leetcode односвязного списка, сделать по
аналогии).
________________________________________________________________________
...
*Куча* (heap) - структура данных, которая позволяет добавить что-то в
мн-во (`add`) и удалить из мн-ва минимальный (куча минимумов) или
максимальный (куча максимумов) эл-т (`pop`). Используется в т.ч. для
...
расширяемом массиве послойной нумерацией эл-тов (0 - корень, 1 и 2 -
дети, 3, 4, 5, 6 - внуки, и т.д.).

Для i-ого эл-та дети будут под индексами 2i + 1 и 2i + 2, а родитель
будет под индексом (i - 1) // 2.

Добавление эл-та (на примере кучи минимумов):

- Добавляем эл-т на первый незаполненный слой в самую левую незанятую
    позицию.
...
Удаление эл-та (на примере кучи минимумов):
...
кэширование самых недавно использованных значений, удаление самого давно
использованного значения. Для этого понадобится словарь для сохранения
значений и куча для времени.
________________________________________________________________________
...
(и упрощения понимания реализации на стандартных структурах данных)
стоит рассмотреть так называемый *менеджер памяти* (memory manager) для
бинарного дерева.
...
### <u> Описание бин. дерева </u> ###

*Бинарное дерево* (binary tree) - структура, представляющая собой набор
узлов, которые связаны друг с другом по следующим правилам:
...
- В общем смысле предоставляет набор правил наследования (см. "Дерево
    ...
    виду *бинарное дерево поиска* (binary search tree, BST), для
    которого правило следующее: в левом поддереве любого узла все ключи
    меньше, чем в данном узле, а в правом - больше.

Корнем называется самая первая добавленная ячейка, а от нее дерево будет
разветвляться далее. Листом называется узел дерева, не имеющий ни одного
сына.
...
2. *Потомок 1*. Тогда у родителя меняем соответствующего сына на этого
    потомка.
...
Отдельная крупная тема, пока что счел более правильным читать отдельно
по необх-ти. Ключевые слова для поиска: АВЛ-дерево, красно-черное
дерево; более экзотические: декартово дерево, сплей-дерево.
________________________________________________________________________

### <u> Дополнительно </u> ###

Одно из возможных представлений дерева в Python:
...
накручивать остальные ф-ции, указанные выше, и, как по мне, является
самым удобным и лучшим вариантом.
________________________________________________________________________
...
В общем смысле дерево есть связный граф (см. "Граф") без циклов. В таком
ключе можно придумать множество различных деревьев. Но среди них
выделяются:
...
- *Бор* (trie) - дерево с подписанными ребрами (см. исп-е в
    алгоритмах Хаффмана, Лемпела-Зива-Велча, а также в особых
    исп-ях связных списков).

- *Корневое дерево* (root tree) - дерево, ориентированное от дочерних
    эл-тов к родителям (см. исп-е в особых исп-ях связных списков).

Другие небинарные деревья также иногда используются. В более общем
понимании небинарное дерево есть много где, например, каталог папок,
файлов и подпапок. Или дерево классов/других эл-тов в программе. Более
неочевидный пример - HTML-документ.

Если реализуется небинарное дерево программно, то сыновья хранятся
списком (см. "Бинарное дерево поиска"). Обходим аналогично бинарному,
просто запуская рекурсивную ф-цию для всех сыновей по списку.
________________________________________________________________________
...
*Граф* (graph) - структура данных, состоящая из вершин и ребер,
связывающих пары вершин. Бинарное дерево - частный пример графа, с
ограничениями на связи между элементами (родители-дети, не более 2
детей). В графе в общем смысле таких ограничений нет, могут возникать
циклы, могут быть разные виды связи по ребрам, нет родителей и детей.
...
- *Разреженность* - антоним полноте. Минимальное число ребер для
    ...
    разреженные матрицы по кол-ву связей: если их больше порядка
    `V**1.5`, то заполненные, а если нет, то разреженные. Очень
    натянутая и условная граница.
...
1. Матрица смежности - матрица V x V, где V - число вершин. В ней
    вертикаль и горизонталь соответствуют номерам вершин, а в полях на
    пересечении i и j указан 0, если вершины не смежны, и 1, если
...
2. Список смежности - список длины V с массивами смежных эл-тов для
    ...
    Чаще используется, т.к. чаще графы разреженные, нежели полные, и
    задачи более подходящие. Далее рассматриваем именно такой вариант
    хранения, если не указано иное.
________________________________________________________________________

### <u> Обход в глубину </u> ###

*Поиск (обход) в глубину* (depth-first search, DFS) - нужен для того,
...
развилки. В коде это реализуется рекурсивно, где углубление в рекурсию и
возврат на ур-нь выше отвечают за запись обратной стрелочки и возврат на
вершину назад по стрелочке соответственно.
...
Проверка графа на двудольность - можно ли разложить граф на 2 доли так,
что ребра будут проходить из одной доли в другую и не будет ребер между
...
*Поиск (обход) в ширину* (breadth-first search, BFS) - в отличие от DFS,
...
- Ищем по всем клеткам ячейки со значением, равным номеру итерации, и
...
(O(V \* E), кроме того с не лучшей константой). Тем не менее, данная
базовая идея хорошо описывает сам принцип, логику и смысл обхода в
ширину.
________________________________________________________________________
...
- Сохранение вершин через очередь.
    ...
    - Обсловлено отсутствием динамически расширяемых массивов в прошлом.
        ...
        Для большей информации по очередям см. "Очередь".
________________________________________________________________________
...
оптимальный, т.к. рассчитывает много ненужных вершин и путей. Для
...
образом, каждой вершине графа будет соответствовать 2 числа. Если сумма
...
некоторая доп. проверка, варианты которой укажем позже, то и ребро лежит
на кратчайшем пути.
...
Во-вторых, какие доп. проверки могут быть, чтобы избежать случая,
указанного выше? Например, проверить также, что для первых (или вторых)
чисел в вершине выполняется равенство их разницы единице (по аналогии с
тем, как восстанавливается кратчайший путь, был бы переход от одной
вершины к другой при восстановлении). Либо, более понятный вариант,
пусть x - первое число левой вершины, а y - второе число правой, k -
длина кратчайшего пути. Тогда должно выполняться равенство
...
*Алгоритм Дейкстры* (Dijkstra algorithm) предназначен для поиска
кратчайшего пути во взвешенном графе. К этому алгоритму постепенно
подходили размышления выше, в 0-k графах в контексте BFS.
...
добраться из начальной точки в целевую нельзя.
...
но не сам маршрут. Для восстановления ответа следует завести доп. массив
prev для хранения номера узла, из которого пришли в текущий в том самом
записанном в dist минимальном случае. Тогда обратной пробежкой (с конца
в начало) ответ восстановится после нахождения минимального пути.
prev[1] = -1 или 0, как удобнее в реализации.
...
помним из рассуждений выше (за исключением допустимости мультиграфа)
составляет не более V\*\*2. Т.о. сложность составляет O(V\*\*2 + E) =
O(V\*\*2).
...
данные подтягиваются и используются. Т.о. сокращаются расчеты проезда по
городам, особенно учитывая то, что в них плотность перекрестков выше,
чем в "пустом поле".
...
удалять по индексу. Вообще это возможно, см. "Куча", но это нестанд.
...
сначала удалить старое, поменять в массиве dist, потом добавить новое.
...
Провести алгоритм Дейкстры из начала в конец и из конца в начало, а
затем перебрать все ребра и применить к ним купон. Тогда сумма затрат на
левом конце ребра по первому запуску алгоритма Дейстры и на правом конце
ребра по второму дадут суммарную стоимость.

**Решение 2**:

Как будто можно по алгоритму Дейкстры тянуть не один пар-р мин. пути, а
пар-р мин. пути без использования ранее купона и пар-р минимального с
использованием. Или через раздваивание вершин на "с купоном" и
"без купона".
________________________________________________________________________
...
Существует *алгоритм Йена* для поиска K минимальных путей от начальной
до конечной точки. Долгий и относительно сложный алгоритм, но можно
...
*Персистентность* (persistence) - сохранение доступа к предыдущим
состояниям системы (например, персистентные структуры данных, git, ф-ция
undo-redo в редакторах). Персистентность называется полной, если доступ
сохраняется ко всем предыдущим состояниям, и при этом во все эти
состояния можно вносить изменения.
...
(j + 1)-ый эл-т массива версий указывает на эл-т X, содержащий в себе x
и указатель на эл-т, на который указывает i-ый эл-т массива версий.

При наличии j версий помимо 0-ой и удалении из i-ой версии эл-та:
(j + 1)-ый эл-т массива версий указывает на эл-т, на который указывал
эл-т, на который указывал i-ый эл-т массива версий.

Для упрощения восприятия возьмем пример:

```
Синтаксис: push(x, n_version), pop(n_version).
p<number> - указатель на число number.
...
Этот способ описания может быть все еще непонятным, по необх-ти - см.
"Тренировки 7.0" от Яндекса, Лекция 4, или поиск в Интернете.
________________________________________________________________________
...
Новая версия (p<number> - указатель на число number из прошлой версии):
...
1. [Сайт] **Метод скользящего окна**:
    [ссылка](https://apptractor.ru/info/techhype/sliding-window.html),
    [ссылка](https://habr.com/ru/articles/347378/).
1. [Сайт] **Алгоритм "Разделяй и властвуй"**:
...
1. [Сайт] **Описание алгоритмов сортировки и сравнение их**
    **производительности**:
...
1. [Сайт] **Алгоритм А\***:
    [ссылка](https://habr.com/ru/articles/331192/),
    [ссылка](https://neerc.ifmo.ru/wiki/index.php?title=Алгоритм_A*&mobileaction=toggle_view_mobile).
1. [Сайт] **Метод Монте-Карло**:
    [ссылка](https://thecode.media/monte-carlo/),
    [ссылка](https://habr.com/ru/articles/274975/).
1. [Сайт] **Непрактичные алгоритмы сортировки для ознакомления**:
    [ссылка](https://habr.com/ru/articles/198114/),
    [ссылка](https://ru.wikipedia.org/wiki/Лас-Вегас_(алгоритм))
