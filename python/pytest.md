...
*Тестирование* - проверка, что продукт работает соответственно ТЗ.

- *QA (Quality Assurance)* - это профилактика. QA отвечает за процессы:
    как организовано тестирование, есть ли стандарты, правила, ревью,
    документация.
- *QC (Quality Control)* - это диагностика. Это уже конкретные проверки,
    тест-кейсы и поиск багов в готовом продукте.
________________________________________________________________________

Тестирование классифицируется по различным критериям и различным
степеням подробности.

Уровни тестирования:

- *Модульное (unit)* - проверяются отдельные ф-ции или классы.
- *Интеграционное (integration)* - как модули взаимодействуют м/ собой.
- *Системное (system)* - вся система целиком, как единый организм.
- *Приемочное (acceptance)* - проверка готового продукта пользователем
    или заказчиком.

По соотношению частоты исп-я:

- Самые частые - unit-тесты - быстрее, дешевле, чаще нужно.
- Средние по частоте - integration-тесты.
- Самые редкие - UI / E2E-тесты (проверка глазами пользователя) -
    медленнее, дороже, редко нужно.

Виды тестирования:

- *Smoke testing* - проверить, что система вообще запускается и не
    падает.
- *Sanity testing* - проверить конкретную исправленную или новую ф-цию.
- *Регрессионное тестирование* - убедиться, что старый функционал не
    сломался.
- *Функциональное тестирование* - проверить выполнение требований к
    продукту (ф-ций продукта).
- *Нефункциональное тестирование* - проверить хар-ки системы.
- *Performance testing* - проверить производительность - скорость и
    стабильность.
- *Load testing* - проверить поведение под нагрузкой.
- *Stress testing* - проверить, где предел прочности.
- *Security testing* - проверка безопасности.
- *Usability testing* - проверить удобство интерфейса.
- *Compatibility testing* - проверить работу в разных предполагаемых
    средах (браузерах, ОС и проч.).
- *Localization testing* - проверка локализации, формата и отображения
    данных на разных языках.
- *Recovery testing* - проверить восстановление после сбоев.
- *Accessibility testing* - проверить доступность для людей с
    ограничениями.
- *Exploratory testing* - исследовать систему без сценария (в свободном
    режиме, чаще всего вручную).
- *Ad-hoc testing* - спонтанная проверка без документации.
- *Alpha testing* - внутренние проверки до релиза, тестирование
    командой QA.
- *Beta testing* - проверки внешними пользователями.

По динамичности:

- *Статическое тестирование* - это анализ без запуска кода (ревью
    требований, кода, документации).
- *Динамическое* - когда мы действительно выполняем тесты, кликаем,
    запускаем автотесты или делаем API-запросы.
________________________________________________________________________

Тестирование - это задекларированный процесс.

Сначала идет *тест-план* - крупный общий план тестирования продукта с
целями, объемами, критериями успешности и завершения, рисками и проч.
Далее идет *чек-лист* - список конкретных проверок без подробностей.
И самая атомарная единица тестирования - *тест-кейс* - конкретный
пошаговый сценарий одной проверки.

По итогу при нахождении багов составляется *баг-репорт* с заголовком,
средой, шагами, ожидаемым и фактическим результатами, приоритетом
исправления, серьезностью и вспомогательными материалами (логами,
скринами, описаниями и т.п.).
...
- *Trivial* - опечатка, визуальный недочет.
________________________________________________________________________

Техники тест-дизайна:

- *Граничные значения (Boundary Value Analysis)* - проверка крайних
    значений диапазона (например, 17, 18, 60, 61 для поля "возраст
    18-60").
- *Разбиение на классы эквивалентности (Equivalence Partitioning)* -
    делим входные данные на допустимые и недопустимые группы.
- *Таблицы решений (Decision Tables)* - когда есть много условий и
    комбинаций.
- *Диаграммы переходов состояний (State Transition)* - когда система
    имеет состояния (логин -> активен -> заблокирован -> разлогинен).
- *Попарное тестирование* - подбор минимального набора тестов,
    покрывающего все пары пар-ров.
- *End-to-end проверки* - проверка работы цепочки действий целиком.
________________________________________________________________________
...
- *pythonpath* - для работы импорта в тестах указывается корневая папка
    проекта относительно места запуска (в хорошей архитектуре - как
    написано в примере выше).
...
```py
def idfn(v) -> str:  # v здесь - принимаемые данные
    ...
...
подготовки к тестам. Эти данные отдаются в тест как аргумент по имени, а
затем, если предусмотрено, фикстура корректно убирается после себя.
...
Отчет можно делать не только в консоль, но и в отдельные репорт-файлы.
Например, с помощью `pytest-html` (`pip install pytest-html`) и флагов
запуска `--html=report.html` и `--self-contained-html`
(например, в pyproject.toml) можно получить отчет в формате HTML в файле
report.html.

По аналогии можно исп-ть JSON-формат отчета с помощью `pytest-json`.
________________________________________________________________________

Для динамически генерируемых тестов может оказаться полезной библиотека
hypothesis. Это библиотека для property-based тестирования в Python. Она
позволяет автоматизировать проверку св-тв кода на мн-ве автоматически
сгенерированных входных данных, что помогает находить скрытые ошибки и
граничные случаи.
________________________________________________________________________
...
1. [Сайт] **Краткая теория тестирования на Хабре**:
    [ссылка](https://habr.com/ru/articles/960202/).
