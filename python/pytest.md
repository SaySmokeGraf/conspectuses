...
библиотеке unittest и призванный упростить тестирование.
...
- *Traceback* - это отчёт о том, по какой цепочке ф-ций Python дошел до
    места, где произошло исключение.
...
- *Mark‑функции* - это ярлыки для тестов в виде ф-ций‑декораторов. Ими
    помечают ф-ции тестов, чтобы исключать/выбирать при запуске, условно
...
- *Моки* - это подменные объекты, которые имитируют поведение внешних
    ...
    вызвал, с какими аргументами), не трогая реальный мир.
________________________________________________________________________
...
- *testpaths* - директории, в которых pytest будет искать тесты. 
- *python_files*, *python_classes*, *python_functions* - шаблоны имён
...
- *norecursedirs* - директории, которые pytest пропустит при сборе
    тестов (исключит из поиска). 
- *cache_dir* - директория для кэша pytest.
________________________________________________________________________
...
*assert* - станд. Python-инструкция проверки соответствия в отладочных и
...
В pyproject.toml есть возможность указать маркеры, которыми затем можно
...
принимает строку с перечислением параметров ф-ции и список с тюплами
...
Проверка поднятия конкретного исключения производится с помощью
контекстного менеджера `pytest.raises`.
...
свои фикстуры и функции в отдельном модуле и импортировать вручную.
...
То есть этот файл - это место где складываются "элементы инфраструктуры"
для выполнения тестов на уровне текущей директории.
________________________________________________________________________
...
return/yield в тест. И после теста/модуля/сессии выполняет teardown,
т.е. всё что после yield или через addfinalizer.
...
вызывает их в определённые моменты (старт сессии, парсинг CLI, коллекция
...
def pytest_configure(config):
    ...
    Создаём временный каталог и сохраняем в объект config.
    """
...
выполняются до и после всей сессии тестирования соотв. Т.е. можно
настроить предварительную подготовку или финальную чистку.

`pytest_runtest_call(item)` - выполняется вокруг тестов, как бы
оборачивая его без декорирования, но с помощью доп. декоратора
...
код до и после выполнения всех остальных обработчиков хука. Это делается
с помощью yield и всё что до yield выполняется перед основной работой
хука, а всё что после yield - после нее. То есть это своеобразный способ
сделать "around" обёртку вокруг хука. С помощью него можно также
контролировать порядок вызова хуков, не будем углубляться в это и идём
дальше.
...
def pytest_runtest_call(item):
    ...
    Обёртка вокруг выполнения самого теста. Код до yield выполняется до
    теста, код после yield - после теста. Это удобное место для
    измерения времени, перехвата исключений и т.п.
    """
    ...
    # Получаем информацию об исполнении (исключение/успех) через
    # outcome.get_result() не даёт report, но мы можем посмотреть, упало
    # ли исключение во время вызова (через outcome.exception() в new
    # pytest?). Простая проверка: если тест выбросил исключение, оно
    # будет проброшено дальше, но мы всё равно логируем.
...
def pytest_runtest_makereport(item, call):
    ...
    if rep.when == "call" and rep.failed:
        # если тест использовал фикстуру 'run_ssh' - добавим её
        # содержимое в секцию отчёта
...
*Мок* - "заглушка" на место реального внешнего объекта или источника,
...
\- встроенный GitLab CI, есть ещё CircleCI, Jenkins и др. Принцип у всех
...
происходить тестирование на Ubuntu след. шагами:
