...
- **[backend](#backend)**
    - **[Django_DRF](#backenddjango_drfmd)**
...
### backend/Django_DRF.md ###

Конспект по фреймворкам Django и Django REST Framework под Python.
________________________________________________________________________

### backend/FastAPI.md ###

Конспект по асинхронному бэкенд фреймворку FastAPI под Python.

Далеко не претендует на полноту, но имеет набор множества необходимых
для работы с ним функций. В основном опирается на официальный туториал.
________________________________________________________________________
...
Для большего понимания краткая справка по сущности DevOps:

*DevOps* (от Development Operations) - это культура, набор практик и
инструментов, а также профессия, которые объединяют разработку ПО и
эксплуатацию продукта. Главная цель - сократить вермя вывода продукта на
рынок, повысить качество и скорость разработки. Основные принципы
описываются моделью CALMS.

DevOps дает:

- *CI/CD* (Continuous Integration / Continuous Delivery) - постоянный
    полный цикл разрабоки продукта и фич к дальнейшей поддержке с
    доставкой до пользователя.
- *IaC* (Infrastructure as Code) - инфраструктура (серверы и окружения)
    описываются как код (конфигурациями).
- Наблюдаемость и мониторинг.
- Наглядная и быстрая обратная связь по работе в реальных условиях.
________________________________________________________________________

### DevOps/docker.md ###

Краткий конспект по Docker. Содержит в себе базовый функционал для
старта использования Docker, Docker Hub, Docker Compose.

Конспект сам по себе крайне краток, но содержателен для простого начала
работы с Docker и его составляющими для разработчика. Для большего
углубления в тему - см. источники в интернете.
________________________________________________________________________
...
Краткий конспект для быстрого входа в фреймворк тестирования pytest.
Содержит основные функции в краткой и грубой форме, для более глубокого
и точного изучения - см. документацию на pytest.
________________________________________________________________________
